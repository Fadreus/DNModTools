...
// in recompression mode I consider MAXZIPLEN as universal for any algorithm, indeed I use /100 instead of /1000
#define MAXZIPLEN(n) ((n)+(((n)/100)+1)+12) // 100 instead of 1000
...
int lzma_compress(unsigned char *in, int insz, unsigned char *out, int outsz, int lzma_flags) {
    printf("\nError: lzma compression not implemented on this system\n");
    myexit(-1);
    return(-1);
}
...
int ungzip(u8 *in, int insz, u8 **ret_out, int *full_outsz) {
    ...
    for(inl -= 4; inl > in; inl--) {  // lame, simple and working
        //fsize = getxx(inl, 4);
        fsize = (inl[0]) | (inl[1] << 8) | (inl[2] << 16) | (inl[3] << 24); // little endian
